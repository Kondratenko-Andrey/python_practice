...
def one_year_of_war():
    ...
    for day in range(1, 366):
        ...
        # В итогах дня у каждого героя и каждого монстра вызывается метод __str__ который описывает их текущее состояние
        print("\nКоманда добра:\n" + '-' * 50)
        for hero in good_team:
            print(hero)
...
# Код запускает 20 симуляций.
count_of_wins = 0
for year in range(1, 21):
    count_of_wins += one_year_of_war()
    print(f'Закончился год: {year}\n'
          f'Кол-во побед: {count_of_wins}')
