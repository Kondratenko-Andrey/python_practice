...
def _sum():
    arr1 = request.form.getlist("array1", type=int)
    arr2 = request.form.getlist("array2", type=int)

    result = ",".join(str(a1 + a2) for a1, a2 in zip(arr1, arr2))
...
def _sum2():
    ...
    for encoded_chunk in request_data.split('&'):
        ...
        arrays[k] = [int(el) for el in v.split(',')]
